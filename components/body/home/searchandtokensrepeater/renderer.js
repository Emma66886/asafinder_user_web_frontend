...
export default function RendersearchandReapeter({ tokens }) {
  ...
  const formItemsData = () => {
    const dataFormed = tokens.map((v) => {});
  };
  const getFullTokenData = async () => {
    return Promise.all(
      tokens.map(async (val) => {
        //   return getAllTokensData(val.token_asa).then((r) => {
        //     console.log({ ...r, ...val });
        //     return { ...r, ...val };
        //   });
        const data = await getAllTokensData(val.token_asa);
        console.log({
          ...data,
          ...val,
        });
    ...
    <>
      {isLargerThan700 && (
        <Box
          ...
          mb={5}
        >
          <Text fontSize="3rem" color="#45AC75" fontWeight={600}>
            TOKEN LIST
          </Text>
          <Input
            w="40%"
      ...
      >
        ...
        >
          {" "}
          List your token now! Get your community to vote for your coin and gain
          exposure.
        </Text>
        <Button
          color="#fff"
      ...
      >
        ...
        <Text
          ...
          color="#fff"
        >
          Create an Account and Sign up to our industry news and Token launch
          list and get your next investment opportunity direct to your inbox on
          a weekly basis.
        </Text>
        <Button
          _hover={{ color: "#000000" }}
